...
void loop() {   //main
  ...
  if (SerialBT.available()) { //if this ESP32 has recieved info to move stuff around 
    ...
    int delimiterIndex = joystickData.indexOf(','); //added
    ...
    int button = joystickData.substring(delimiterIndex2 + 1).toInt(); //added not sure of it 
    ...
    // Move to left - decrease left motor speed, increase right motor speed
      ...
      const int speedThreshold = 70; //added but maybe be changed 
